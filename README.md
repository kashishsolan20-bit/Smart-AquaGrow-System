...
Based on the received input, the system evaluates whether the soil is dry or sufficiently moist. If the moisture level falls below a predefined threshold, the Arduino activates the relay module, which in turn switches the water pump ON. Once the desired moisture level is reached, the system automatically turns the pump OFF.
This process ensures efficient water usage by supplying water only when required, thereby maintaining optimal soil conditions.
...
# Conclusion
Smart AquaGrow demonstrates how simple automation can effectively solve everyday problems like plant watering. By combining sensing and control, the system ensures plants receive the right amount of water at the right time without manual effort.
This project highlights the importance of efficiency, reliability, and practical implementation, making it a useful solution for home environments. It also serves as a foundation for building more advanced smart systems in the future.
